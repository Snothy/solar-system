...
function getBodyColor(data: CelestialBodyData): string {
  ...
  return (
    ORBIT_COLORS[data.name] || `#${new THREE.Color(data.color).getHexString()}`
  );
}
...
export function OrbitEllipse({
  ...
  const line = useMemo(() => {
    ...
    const G = 6.6743e-11;
    const mu = G * parentVisualBody.body.gm;
    ...
    visualBody.body.pos,
    visualBody.body.vel,
    parentVisualBody?.body.pos,
    parentVisualBody?.body.vel,
    data.name,
    visible,
  ]);
