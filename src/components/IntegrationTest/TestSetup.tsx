...
import { listSnapshots, formatEpochDate, timeAgo } from '../../services/snapshotStorage';
import { listTestRuns, deleteTestRun, loadTestRun } from '../../services/testStorage';
...
export function TestSetup({ onRun, onReplay, onOpenSettings, activeSettings }: Props) {
  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%', gap: '0' }}>
      ...
      <div className="apple-panel" style={{
        width: '380px', flexShrink: 0, borderRadius: '0', height: '100%',
        ...
        {/* Archive selector */}
        <div>
          <div style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.1em', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginBottom: '12px' }}>
            Archive
          </div>
          ...
          {archives.length === 0 ? (
            <div style={{
              ...
              fontSize: '13px', color: 'rgba(251,146,60,0.9)', lineHeight: '1.5',
            }}>
              No archives yet. Pull one in
              <span style={{ color: '#fb923c', fontWeight: 600 }}> Settings → Data</span>
              .
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {archives.map(a => (
                <button
                  key={a.id}
                  onClick={() => setSelectedArchiveId(a.id === selectedArchiveId ? null : a.id)}
                  style={{
                    padding: '12px 14px', borderRadius: '10px', textAlign: 'left',
                    background: a.id === selectedArchiveId
                      ? 'rgba(255,255,255,0.12)'
                      : 'rgba(255,255,255,0.04)',
                    border: a.id === selectedArchiveId
                ...
                >
                  ...
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.45)', display: 'flex', gap: '8px' }}>
                    <span>{formatEpochDate(a.startEpoch.date)} → {formatEpochDate(a.endEpoch.date)}</span>
                  </div>
        ...
        <div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Integrator</span>
              <span style={{ fontSize: '12px', fontWeight: 500, color: '#fff' }}>
                {INTEGRATOR_LABELS[activeSettings.integratorMode] ?? activeSettings.integratorMode}
              </span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Quality</span>
              <span style={{ fontSize: '12px', fontWeight: 500, color: '#fff' }}>{activeSettings.quality}</span>
            </div>
            <div style={{ height: '1px', background: 'rgba(255,255,255,0.06)' }} />
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              <TogglePill label="Relativity" active={activeSettings.enableRelativity} />
              <TogglePill label="Tidal" active={activeSettings.enableTidalEvolution} />
              <TogglePill label="Atm Drag" active={activeSettings.enableAtmosphericDrag} />
              <TogglePill label="Yarkovsky" active={activeSettings.enableYarkovsky} />
              <TogglePill label="GravHarmonics" active={activeSettings.enableGravitationalHarmonics} />
              <TogglePill label="SRP" active={activeSettings.enableSolarRadiationPressure} />
            </div>
            <button
              onClick={onOpenSettings}
              style={{
                alignSelf: 'flex-start', padding: '4px 0', fontSize: '12px',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '32px', overflowY: 'auto' }}>
        <div style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.1em', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginBottom: '20px' }}>
          Test History
        </div>

        {history.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.2)', fontSize: '14px' }}>
...
function HistoryRow({
  ...
  const errorColor =
    meanKm < 100 ? '#4ade80' :
    meanKm < 1000 ? '#facc15' : '#f87171';

  const integratorLabel: Record<string, string> = {
    saba4: 'SABA4', adaptive: 'DOP853',
    'wisdom-holman': 'W-H', 'high-precision': 'High-P', standard: 'Std',
  };
  ...
  return (
    ...
    >
      {/* Error indicator dot */}
      <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: errorColor, flexShrink: 0 }} />
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <span style={{ color: 'rgba(255,255,255,0.25)' }}>{timeAgo(run.createdAt)}</span>
        </div>
      </div>
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        ...
        <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)' }}>mean error</div>
      </div>
      ...
      <button
        ...
        style={{ background: 'transparent', color: 'rgba(255,255,255,0.3)', padding: '4px 6px', fontSize: '16px', flexShrink: 0 }}
        title="Delete"
      >
        ×
      </button>
