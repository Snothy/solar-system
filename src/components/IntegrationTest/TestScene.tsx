...
function calcRadius(
  ...
  let r = (b.equatorialRadius ?? 1e6) * SCALE;
  if (useVisualScale) {
    r *= visualScale;
    if (r > 40) r = 40;
  }
...
const BODY_STATIC = new Map(
  ALL_BODIES.map((b) => [
    b.name,
    {
      ...
      baseRadius: (b.equatorialRadius ?? 1e6) * SCALE, // unscaled, for camera focus logic
    },
  ]),
);
...
function BodyTrail({
  ...
  const { simLine, refLine } = useMemo(() => {
    ...
    const simPts = slice
      .map((f) => f.bodies[name]?.simPos)
      .filter(Boolean)
      .map((p) => toScene(p as [number, number, number]));
    const refPts = slice
      .map((f) => f.bodies[name]?.refPos)
      .filter(Boolean)
      .map((p) => toScene(p as [number, number, number]));
    ...
    return {
      simLine: makeL(simPts, colorHex, 0.65),
      refLine: makeL(refPts, "#fb923c", 0.55),
    };
...
function CameraController({
  ...
  useEffect(() => {
    focusedBodyRef.current = focusedBody;
  ...
  useEffect(() => {
    ...
    const mid =
      errScene > MIN_VISIBLE_ERROR_SCENE
