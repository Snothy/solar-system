...
export function App() {
  ...
  const [activeSnapshot, setActiveSnapshot] = useState<SnapshotData>(
    baselineSnapshot as unknown as SnapshotData,
  );
  const [mergedBodies, setMergedBodies] = useState<any[]>(
    mergeBodiesWithSnapshot(
      SOLAR_SYSTEM_DATA as any[],
      baselineSnapshot as unknown as SnapshotData,
    ),
  );
    ...
    getLatestSnapshot()
      .then((record) => {
        if (record) {
          ...
          setMergedBodies(
            mergeBodiesWithSnapshot(SOLAR_SYSTEM_DATA as any[], record),
          );
        }
      })
      .catch(() => {
        /* no IDB data yet, use baseline */
      });
  }, []);
  ...
  const handleSnapshotLoaded = (snapshot: SnapshotData) => {
    ...
    const newBodies = mergeBodiesWithSnapshot(
      SOLAR_SYSTEM_DATA as any[],
      snapshot,
    );
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 50 }}>
          ...
          >
            ...
            >
              ...
              >
                {(["realistic", "simplistic", "test"] as ViewMode[]).map(
                  (mode) => (
                    <button
                      key={mode}
                      ...
                      style={{
                        ...
                        background:
                          viewMode === mode
                            ...
                            : "transparent",
                        color:
                          viewMode === mode ? "#fff" : "rgba(255,255,255,0.5)",
                        boxShadow:
                          viewMode === mode
      ...
      {viewMode !== "test" && (
        <div className="ui-layer">
          ...
          >
            ...
            {performanceMetrics.avgFrameTime.toFixed(1)}ms
          </div>
