...
export function useSimulation(
  ...
  const [liveData, setLiveData] = useState<SolarSystemData[] | null>(
    initialData as SolarSystemData[] | null,
  );
      ...
      try {
        ...
        liveData.forEach((data) => {
          ...
          if ((!initialPos || !initialVel) && data.rel_a) {
            ...
            if (parentBody) {
              const mu = 6.6743e-11 * parentBody.gm;
              const a = data.rel_a;
              ...
              const x_ref =
                x_orb * (cos_node * cos_peri - sin_node * sin_peri * cos_i) -
                y_orb * (cos_node * sin_peri + sin_node * cos_peri * cos_i);
              const y_ref =
                x_orb * (sin_node * cos_peri + cos_node * sin_peri * cos_i) +
                y_orb * (sin_node * sin_peri - cos_node * cos_peri * cos_i);
              const z_ref =
                x_orb * (sin_peri * sin_i) + y_orb * (cos_peri * sin_i);

              const vx_ref =
                vx_orb * (cos_node * cos_peri - sin_node * sin_peri * cos_i) -
                vy_orb * (cos_node * sin_peri + sin_node * cos_peri * cos_i);
              const vy_ref =
                vx_orb * (sin_node * cos_peri + cos_node * sin_peri * cos_i) +
                vy_orb * (sin_node * sin_peri - cos_node * cos_peri * cos_i);
              const vz_ref =
                vx_orb * (sin_peri * sin_i) + vy_orb * (cos_peri * sin_i);
              ...
              const pos_final = X_eq.clone()
                .multiplyScalar(x_ref)
                .add(Y_eq.clone().multiplyScalar(y_ref))
                .add(Z_eq.clone().multiplyScalar(z_ref));

              const vel_final = X_eq.clone()
                .multiplyScalar(vx_ref)
          ...
          const body: PhysicsBody = {
            ...
            poleRARate: data.poleRA_rate,
            poleDecRate: data.poleDec_rate,
            ...
            momentOfInertia:
              0.4 *
              (data.gm / 6.6743e-11) *
              data.equatorialRadius *
              data.equatorialRadius, // Solid sphere approximation
          ...
          const visualBody: VisualBody = {
            ...
            trailIdx: 0,
            trailCount: 0,
            baseRadius: data.equatorialRadius * SCALE,
            type: data.type,
      ...
      prevVisualBodies.map((vb) => {
        if (vb.body.name === name) {
          ...
          return {
            ...vb,
            baseRadius:
              updates.equatorialRadius !== undefined
                ? updates.equatorialRadius * SCALE
                : vb.baseRadius,
            body: newBody,
          };
        }
    ...
    const newBody: PhysicsBody = {
      ...
      poleVector:
        data.poleRA !== undefined && data.poleDec !== undefined
          ? computePoleVector(data.poleRA, data.poleDec)
          : new THREE.Vector3(0, 1, 0),
      momentOfInertia: 0.4 * (data.gm / 6.6743e-11) * data.radius * data.radius,
      angularVelocity: data.rotationPeriod
        ? (data.poleRA !== undefined && data.poleDec !== undefined
            ? computePoleVector(data.poleRA, data.poleDec)
            : new THREE.Vector3(0, 1, 0)
  ...
  const updatePhysics = () => {
    ...
    const targetDt = !physics.isPaused
      ? (physics.timeStep === 0 ? 1 : physics.timeStep) / 60
      : 0;
