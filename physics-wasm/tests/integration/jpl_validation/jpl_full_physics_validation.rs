...
fn run_jpl_validation(
    ...
    let jpl_data = load_jpl_vector(body_name)
        .expect(&format!("No JPL data for {}", body_name));
    
    if jpl_data.is_empty() || jpl_data.len() < duration_hours {
        panic!("{}: Insufficient JPL data (need {} hours, have {})", 
            body_name, duration_hours, jpl_data.len());
    }
...
fn print_result(result: &JPLValidationResult) {
    ...
    println!("Duration: {} hours ({} days)", result.duration_hours, result.duration_hours / 24);
...
fn generate_comprehensive_jpl_report() {
    ...
    for body in &bodies {
        ...
        report.push_str("|--------|------|-----|-------|----------|\n");
        report.push_str(&format!("| Position (km) | {:.2} | {:.2} | {:.2} | {} |\n",
            result.mean_pos_error_km, result.max_pos_error_km, result.final_pos_error_km,
            if body.contains("Jupiter") { "1000" } else { "100" }));
        report.push_str(&format!("| Velocity (m/s) | {:.4} | {:.4} | {:.4} | {} |\n\n",
            result.mean_vel_error_ms, result.max_vel_error_ms, result.final_vel_error_ms,
    ...
    let mut file = File::create("COMPREHENSIVE_JPL_REPORT.md")
