...
use common::{load_bodies, load_jpl_vector};
...
fn test_solar_system_accuracy_vs_jpl() {
    ...
    let jpl_data = load_jpl_vector("Earth")
        .expect("Failed to load JPL data for Earth");
    ...
    assert!(max_pos_error < 1e6, "Position error too large: {} km", max_pos_error / 1000.0);
}
...
fn test_multiple_bodies_vs_jpl() {
    ...
    for body_name in &test_bodies {
        let body_errors: Vec<f64> = results.iter()
...
fn generate_comprehensive_test_report() {
    ...
    report.push_str("This report contains comprehensive test results for the Rust physics engine.\n\n");
    ...
    let mut file = File::create("test_report.md")
        .expect("Failed to create test_report.md");
