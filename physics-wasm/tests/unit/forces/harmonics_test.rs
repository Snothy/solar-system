...
use physics_wasm::forces::gravity::{apply_c22_s22, apply_j3, apply_j4};
...
fn test_j3_perturbation() {
    ...
    println!(
        "J3 force components: x={:.6e}, y={:.6e}, z={:.6e}",
...
fn test_j4_perturbation() {
    ...
    jupiter.gravity_harmonics = Some(physics_wasm::common::types::HarmonicsParams {
        j4: Some(-5.87e-4), // Jupiter's J4 is significant
        pole_vector: Some(Vector3::new(0.0, 0.0, 1.0)),
        ..Default::default()
    });
    ...
    println!(
        "J4 force components: x={:.6e}, y={:.6e}, z={:.6e}",
        force.x, force.y, force.z
    );
}
...
fn test_c22_s22_harmonics() {
    ...
    earth.radius = 6378137.0;
    earth.gravity_harmonics = Some(physics_wasm::common::types::HarmonicsParams {
        c22: Some(2.43e-6), // Earth's C22
        s22: Some(-1.4e-6), // Earth's S22
        pole_vector: Some(Vector3::new(0.0, 0.0, 1.0)),
        ..Default::default()
    });
    earth.pos = Vector3::zero();
    ...
    println!(
        "C22/S22 force components: x={:.6e}, y={:.6e}, z={:.6e}",
        force.x, force.y, force.z
    );
}
