...
use physics_wasm::forces::relativity::{apply_relativity_ppn, apply_relativity_eih};
...
fn test_ppn_mercury_perihelion_precession() {
    ...
    let (force_on_sun, force_on_mercury) = apply_relativity_ppn(
        &sun, &mercury, &r_vec, dist, dist_sq
    );
    ...
    assert!(relativistic_ratio > 1e-10, "Relativistic correction too small");
    assert!(relativistic_ratio < 1e-5, "Relativistic correction too large");
}
...
fn test_eih_mercury() {
    ...
    let (force_on_sun, force_on_mercury) = apply_relativity_eih(
        &sun, &mercury, &r_vec, dist, dist_sq
    );
...
fn test_ppn_vs_eih_comparison() {
    ...
    assert!(ppn_mag > 0.0 && eih_mag > 0.0, "Both forces should be non-zero");
}
...
fn test_relativity_is_small_correction() {
    ...
    assert!(ratio < 1e-6, "Relativistic correction should be small for Earth");
    assert!(ratio > 1e-12, "Relativistic correction should be measurable");
}
