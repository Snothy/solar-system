...
use physics_wasm::forces::calculate_accelerations;
use physics_wasm::common::constants::G;
use approx::assert_relative_eq;

fn create_test_system() -> (Vec<PhysicsBody>, Vec<Option<usize>>) {
    ...
    // Indices: 0, 1, 2
    let parent_indices = vec![None, Some(0), Some(1)];
...
fn test_newtonian_gravity_full() {
    ...
    let accs = calculate_accelerations(
        ...
        true, // include_sun_gravity
        false // subtract_parent_gravity
    );
...
fn test_wisdom_holman_mode() {
    ...
    let accs = calculate_accelerations(
        &bodies, &parent_indices,
        false, false, false, false, false, false, false, false, false,
        true, // include_sun_gravity = true (Required to enter the block that handles Tidal forces)
        true   // subtract_parent_gravity = true
    );
...
fn test_j2_enable() {
    ...
    // Enable J2 on Earth
    bodies[1].j2 = Some(0.001);
    bodies[1].radius = 6378137.0;
    bodies[1].pole_vector = Some(Vector3::new(0.0, 0.0, 1.0));
    
    let accs = calculate_accelerations(
        &bodies, &parent_indices,
        false, true, // enable_j2
