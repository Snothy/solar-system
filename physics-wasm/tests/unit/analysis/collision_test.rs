...
use physics_wasm::analysis::{check_collisions, resolve_collisions};
...
fn test_collision_momentum_conservation() {
    ...
    println!("Initial momentum: ({:.3e}, {:.3e}, {:.3e})", 
        p_initial.x, p_initial.y, p_initial.z);
    println!("Final momentum:   ({:.3e}, {:.3e}, {:.3e})", 
...
fn test_equal_mass_head_on_collision() {
    ...
    println!("Merged body velocity: ({:.3}, {:.3}, {:.3})", 
        bodies[0].vel.x, bodies[0].vel.y, bodies[0].vel.z);
    
    assert!(bodies[0].vel.len() < 1.0, 
