...
fn test_barycenter_stability() {
    ...
    println!("Initial CM: ({:.3e}, {:.3e}, {:.3e}) m", cm0.x, cm0.y, cm0.z);
    println!("Final CM:   ({:.3e}, {:.3e}, {:.3e}) m", cm1.x, cm1.y, cm1.z);
    println!("Drift:      ({:.3e}, {:.3e}, {:.3e}) m", drift.x, drift.y, drift.z);
    ...
    assert!(drift_per_year < 1000.0, "Barycenter drift too large: {:.3} km/year", drift_per_year);
}
