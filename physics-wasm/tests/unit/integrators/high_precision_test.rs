...
fn test_high_precision_two_body() {
    ...
    for _ in 0..(10 * 24) {
        step_high_precision(&mut bodies, &parent_indices, dt, 0.0,
    ...
    println!("High Precision - Energy drift:   {:.6e} ({:.4}%)", drift, drift * 100.0);
    
    // DOP853 is adaptive 8th order - should have excellent energy conservation
    assert!(drift < 1e-6, "High precision energy drift too large: {:.6e}", drift);
}
...
fn test_high_precision_vs_saba4_accuracy() {
    ...
    let sun_idx = bodies_template.iter().position(|b| b.name == "Sun").unwrap();
    let earth_idx = bodies_template.iter().position(|b| b.name == "Earth").unwrap();
    ...
    let pos_diff = bodies_hp[earth_idx_hp].pos.distance_to(&bodies_saba4[earth_idx_saba4].pos);
    let vel_diff = bodies_hp[earth_idx_hp].vel.distance_to(&bodies_saba4[earth_idx_saba4].vel);
    
    println!("Position difference HP vs SABA4: {:.3} km", pos_diff / 1000.0);
...
fn test_high_precision_large_timestep() {
    ...
    assert!(drift < 1e-5, "High precision should handle large timesteps well");
}
